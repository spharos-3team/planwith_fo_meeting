...
public class OpenApiConfig {
	...
	@Bean
	public OpenAPI planwithFoMeetingOpenAPI() {
		return new OpenAPI()
				.info(new Info()
						.title("PlanWith planwith-fo-meeting API")
						.description("""
								Meeting API. Access Token 검증은 Gateway.
								...
								Meeting을 `:8086`으로 직접 칠 때만 `X-Auth-User-Id`에 memberUuid를 넣는다.
								""")
						.version("v1"))
				...
				.components(new Components()
						...
						.addSecuritySchemes(GATEWAY_USER_ID_SCHEME, new SecurityScheme()
								...
								.description("Meeting 직접 호출(`:8086`)용. Gateway 경유 시 이 헤더는 버려진다")));
	}
}
